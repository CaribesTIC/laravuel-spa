...
export default () => {
  ...
  const {
    ...
    setSearch,
    setSort, 
  } = useTableGrid(data, "/dependencies")

  const getDependencies = (routeQuery: string) => {
    return DependencyService.getDependencies(routeQuery)
      .then((response) => {
        errors.value = {}
  ...
  const deleteRow = (rowId?: string) => {
    if (rowId === undefined)
      ...
      return DependencyService.deleteDependency(rowId)
        .then((response) => {
          errors.value = {}
          getDependencies(
