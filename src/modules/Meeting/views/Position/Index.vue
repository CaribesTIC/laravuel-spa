...
// @ts-nocheck
import useIndex from "../../composables/Position/useIndex";
...
<template>
<div>
  <AppPageHeader> Positions </AppPageHeader>

  <div class="flex space-x-2">
    <AppLink class="btn btn-primary" to="/positions/create">
      <span>Crear</span>
    </AppLink>
  </div>
      ...
      <table class="table-data">
        ...
        <tbody>
          <tr v-for="row in data.rows" :key="row.id" class="">
             <td class="">
              <AppLink
                ...
                :to="{ name: 'positionEdit', params: { id: row.id }}"
              >
                {{ row.id }}
              </AppLink>
            </td>
            ...
            <td class="">
              <div class="flex items-center space-x-1">                
                <AppBtn
                  ...
                  @click="router.push({ path: '/positions/edit/'+row.id })"
                >
                  Editar
                </AppBtn>
                <AppBtn
          ...
          <tr v-if="data.rows.length === 0">
            <td class="" colspan="4">Positions not found.</td>
          </tr>
        </tbody>
      </table>
    </div>
